...
* Access to the AWS account that stores the curated TikTok scrapes. The IAM identity must have permission to `s3:GetObject` (and optionally `s3:ListBucket`) on the curated bucket.
* The curated dataset produced during Section 2 (either as local files or under an S3 prefix). Each row must retain the `raw` JSON payload so the downstream stages can consume the full metadata.
...
This exposes the `classification-pipeline` CLI and pulls in `boto3` for S3 access.
...
1. `io.load_records` resolves every curated batch (streaming from S3 if needed) and converts the payloads into `VideoRecord` dataclasses. Records whose `play_count` is below the `--min-views` threshold are discarded.
...
## 4. Integrating with the existing orchestration
...
## 5. Extending the NLP components
